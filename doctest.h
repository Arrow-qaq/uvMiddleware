...
/*README：添加宏：NOMINMAX和DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN
cmake添加：
add_definitions(-DNOMINMAX) msvc下添加
add_definitions(-DDOCTEST_CONFIG_IMPLEMENT) 每个文件使用时添加
使用例子：
    ...
    FatalConditionHandler()
    {
        ...
        std::set_terminate(
            []() DOCTEST_NOEXCEPT
            {
                ...
                if (isDebuggerActive() && !g_cs->no_breaks)
                    DOCTEST_BREAK_INTO_DEBUGGER();
                std::exit(EXIT_FAILURE);    // explicitly exit - otherwise the SIGABRT handler may
                                            // be called as well
            });
